String authURI = 'http://auth.uniplanet-back.autos/api/auth';
String productURI = 'http://products.uniplanet-back.autos/api/products';
String accountURI = 'http://account.uniplanet-back.autos/api/account';
String chatURI = 'http://chat.uniplanet-back.autos/api/chat';
String messageURI = 'http://message.uniplanet-back.autos/api/message';


// String authURI = 'http://auth.uniplanet.com:3000/api/auth';
// String productURI = 'http://products.uniplanet.com:3001/api/products';
// String chatURI = 'http://chat:3002.com';

// String authURI = 'http://auth.uniplanet.com/api/auth';
// String productURI = 'http://products.uniplanet.com/api/products';
// String chatURI = 'http://chat.com';
